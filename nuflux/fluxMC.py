...
class MuonDecay(object):
    ...
    def simulate_decays(
        ...
        self.df_gen = pd.concat([df_plus, df_minus], axis=0).reset_index(drop=True) #adds all helicities together
    ...
    def propagate_to_detector(
        ...
        self.pnue = self.df_gen["P_decay_nu_e"].to_numpy() #all emitted electron neutrinos momenta
        self.pnumu = self.df_gen["P_decay_nu_mu"].to_numpy() #all emitted muonic neutrinos momenta
