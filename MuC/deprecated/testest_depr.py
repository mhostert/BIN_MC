...
def SimulateDecays(param="mutristan s", N_evals=1e5, alr_loaded=False, dt=None):
    ...
    if not alr_loaded:
        ...
        while attempt < max_attempts:
            ...
            try:
                dt = list(musim.get_particles(param, N_evals))
...
class SimNeutrinos:
    ...
    def __init__(self, coord_object, geom, particle=None, Lss=0, direc="left"):
        ...
        self.w = coord_object.weights.reshape(
...
def load_data(fil, direc=None, n_events=1e5):
    ...
    if directory is None:
        raise ValueError(
            f"Environment variable {direc} is not set. Ensure it's defined in your .bashrc and loaded correctly."
        )
