...
from MuC.detector_tools import face, decayer, initializer, conic, barrel, cap
from MuC import detector_tools as det

# these densities will be number densities
muon_detector_hcal = face(det.Fe.N)
solenoid = face(det.Al.N)
ecal = face(det.WSi2.N)  # should actually use SiW, not WSi2
tracker = face(det.Si.N)
cone = face(det.W.N)
beampipe = face(0)
outside = face(0)
