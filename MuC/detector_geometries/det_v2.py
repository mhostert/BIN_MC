...
muon_detector = face(det.Fe.N)
solenoid_borders = face(det.Fe.N)
solenoid_mid = face(det.Al.N)
hcal = face(det.hcal_CLICdet.N)  # Fe absorber, middle parts comple
ecal = face(
    det.ecal_CLICdet.N
...
three = initializer(beampipe, 3, [15, 16, 17, 18], 0.0, 2.2)


four = barrel(outside, 4, [], 645.0, -563.8, 563.8)
five = cap(outside, 5, [], 563.8, 78.2, 645.0)
...
eightyeight = cap(muon_detector, 88, [4, 7, 93], -417.9, 446.1, 645.0)
eightynine = cap(space_between, 89, [23, 28, 30, 31], -412.9, 324.6, 348.3)
ninety = cap(space_between, 90, [24, 31, 32], -412.9, 352.3, 364.9)
ninetyone = cap(space_between, 91, [25, 31, 33], -412.9, 399.3, 425.0)
ninetytwo = cap(space_between, 92, [26, 31, 34], -412.9, 429.0, 446.1)
ninetythree = cap(muon_detector, 93, [4, 5], 417.9, 446.1, 645.0)
...
twentytwo = cap(solenoid_borders, 22, [60, 61, 62], -412.9, 425.0, 429.0)
twentythree = barrel(solenoid_borders, 23, [54, 56], 348.3, -412.9, 412.9)
twentyfour = barrel(solenoid_mid, 24, [57, 59], 364.9, -412.9, 412.9)
twentyfive = barrel(solenoid_borders, 25, [60, 62], 425.0, -412.9, 412.9)
twentysix = barrel(muon_detector, 26, [4, 93], 446.1, -417.9, 417.9)
...
thirtyfour = barrel(solenoid_borders, 34, [60, 61, 62], 429.0, -412.9, 412.9)

thirtyfive = cap(ecal, 35, [38, 39, 40, 67], -250.9, 33.9, 170.0)
thirtysix = cap(space_between, 36, [29, 71], -235.4, 170.0, 324.6)
thirtyseven = barrel(space_between, 37, [23, 29], 324.6, -412.9, -235.4)
thirtyeight = cap(
    space_between, 38, [41, 42, 43, 44, 69, 70, 71, 72, 87], -230.7, 31.0, 170.0
)
thirtynine = barrel(hcal, 39, [36], 170.0, -250.9, -235.4)
forty = barrel(space_between, 40, [23, 29, 69, 71], 170.0, -235.4, -230.7)
...
fortyeight = conic(
    space_between, 48, [31], (57.5 - 56.8) / (417.9 - 412.9), 412.9, 417.9, 56.8, "away"
)
fortynine = conic(
...
fiftytwo = barrel(ecal, 52, [38, 67], 170.0, -250.9, -235.4)
...
sixty = barrel(space_between, 60, [26, 31], 429.0, -412.9, 412.9)
sixtyone = barrel(space_between, 61, [25, 31, 33], 425.0, -412.9, 412.9)
sixtytwo = cap(space_between, 62, [31], 412.9, 425.0, 429.0)

sixtythree = barrel(space_between, 63, [23, 31], 333.0, -221.0, 221.0)
sixtyfour = cap(space_between, 64, [23, 31, 72], 221.0, 174.0, 333.0)
sixtyfive = barrel(space_between, 65, [70, 72, 83], 174.0, -221.0, 221.0)
...
sixtynine = cap(ecal, 69, [73, 74, 84], -221.0, 150.0, 170.2)
seventy = barrel(ecal, 70, [73, 74, 84], 170.2, -221.0, 221.0)

seventyone = copy.deepcopy(sixtyfive)
seventyone.id = 71
seventyone.density = hcal.density
seventyone.next_ids = [63, 64]
seventytwo = cap(hcal, 72, [79, 80, 85], 235.4, 170.0, 324.6)
seventythree = copy.deepcopy(seventy)
seventythree.id = 73
seventythree.density = space_between.density
seventythree.next_ids = [71, 72]

seventyfour = cap(space_between, 74, [42, 71, 72], 221.0, 150.0, 170.2)
seventyfive = barrel(space_between, 75, [72], 170.0, 230.7, 235.4)
seventysix = barrel(hcal, 76, [79, 80], 170.0, 235.4, 250.9)
seventyseven = cap(hcal, 77, [79, 80, 85], 250.9, 33.9, 170.0)
